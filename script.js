...
async function submitFinalOrder(event) {
    ...
    try {
        ...
        // ✅ აი ეს ნაწილი დაემატა: შეკვეთის შენახვა აპლიკაციაში (Orders ტაბისთვის)
        const newOrder = {
            id: Math.floor(Math.random() * 10000), // გენერირებული ID
            timestamp: new Date().toLocaleTimeString(),
            items: Object.values(cart), // ინახავს პროდუქტების ობიექტებს ჩეკისთვის
            total: total + ' ₾',
            address: street,
            status: 'pending' // საწყისი სტატუსი
        };
        
        myOrders.unshift(newOrder); // ამატებს სიის დასაწყისში
        renderOrders(); // ანახლებს "შეკვეთების" გვერდს
